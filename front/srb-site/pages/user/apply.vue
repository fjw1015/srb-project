...
export default {
  ...
  //created在后端会报错(但是也可以使用)：没有获取到token（用户未登录）  mounted会直接在浏览器端执行 必能获取到token 为了优化 写mounted
  /* mounted() {
    //  获取借款额度
  ...
  watch: {
    "borrowInfo.amount"(value) {
      if (value > this.borrowAmount) {
        //定义变量 callback作用域在自己{}内 无法直接使用this
        let _this = this;
        this.$alert("您的借款额度超出范围", {
          type: "error",
          //错误信息弹出 再次进行回调 将输入的额度改成最大额度
          callback() {
