...
public class Swagger2Config {
    ...
    public Docket adminApiConfig() {
        return new Docket(DocumentationType.SWAGGER_2).groupName("admin").apiInfo(adminApiInfo())
                .select().paths(Predicates.and(PathSelectors.regex("/admin/.*"))).build();
    }

    @Bean
    public Docket webApiConfig() {
        return new Docket(DocumentationType.SWAGGER_2).groupName("web").apiInfo(webApiInfo())
    ...
    private ApiInfo adminApiInfo() {
        return new ApiInfoBuilder().title("尚融宝后台管理系统API文档")
                ...
                .version("1.6")
                .contact(new Contact("fjw", "https://blog.csdn.net/qq_43542647?spm=1000.2115.3001.5343&type=blog", "15679698695@163.com"))
                .build();
    }
