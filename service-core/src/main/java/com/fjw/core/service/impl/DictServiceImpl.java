...
import com.fjw.core.listener.ExcelDictDTOListener;
...
public class DictServiceImpl extends ServiceImpl<DictMapper, Dict> implements DictService {
    ...
    public List<Dict> listByParentId(Long parentId) {
        //首先查询redis中是否存在数据列表
        try {
            List<Dict> dictList = (List<Dict>) redisTemplate.opsForValue().get("src:core:dictList:" + parentId);
            if (dictList != null) {
        ...
        //填充hasChildren字段
        dictList.forEach(dict -> {
            //判断当前结点是否有子节点 找到当前的dict下级有没有子节点
